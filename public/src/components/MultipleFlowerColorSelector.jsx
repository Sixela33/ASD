    ...
    return (
        <div className={`flex items-center ${isListBelow ? 'flex-col' : 'flex-row'}`}>
            ...
            />
            <div className={`grid  overflow-y-auto h-20 w-40 ${isListBelow ? '' : 'flex-col gap-2'}`}>
                {selectedColors.map((item) => (
                <div onClick={() => handleRemoveColor(item)} key={item.colorname} className="bg-gray-200 rounded-md px-2 py-1 flex items-center space-x-2">
